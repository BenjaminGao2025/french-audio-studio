...
- 📚 **句子深度拆解与三列精析 (`/study`)**：
  - **第 1 列（原形/词组）**：自动切分单词与常见复合词组（如 `de temps en temps`, `avoir besoin de`），标注词性（POS）、动词原形（Lemma）与国际音标（IPA）。
...
- 🎙️ **跟读打分与发音复述（Shadowing）**：
  ...
  - **逐词视觉着色反馈**：绿色（准确）、黄色（轻微差异/联诵）、红色（漏读或读错）。
...
- 📦 **轻量可移植**：
  - 基于 FastAPI + 原生现代化前端，无重型框架依赖，启动毫秒级，附带 38 项自动化测试。
